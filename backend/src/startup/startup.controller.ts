...
import { UpdateStartupDto } from '../dto/updateStartup.dto';
  ...
  export class StartupController {
    ...
    @Get()
    @ApiOperation({ summary: 'Get all startups', description: 'Retrieve a list of all startups.' })
    @ApiResponse({
      status: 200,
    ...
    @Get('/email')
    @ApiOperation({ summary: 'Get startup by email', description: 'Retrieve an startup by their email address.' })
    @ApiQuery({
      name: 'email',
      required: true,
    ...
    async getStartupByEmail(@Query('email') email: string) {
      ...
      try {
        ...
        throw new InternalServerErrorException(
          'Error fetching startup by email',
        );
      }
    }

    @Get('/industry')
    @ApiOperation({ summary: 'Get startups by industry', description: 'Retrieve startups based on their industry.' })
    @ApiQuery({
      name: 'status',
      required: true,
    ...
    @Get('/:id')
    @ApiOperation({ summary: 'Get startup by ID', description: 'Retrieve a specific startup by their unique ID.' })
    @ApiParam({
      name: 'id',
      required: true,
    ...
    @Put('/:id')
    @ApiOperation({ summary: 'Update startup', description: 'Update the details of an existing startup.' })
    @ApiParam({
      name: 'id',
      required: true,
    ...
    @Delete('/:id')
    @ApiOperation({ summary: 'Delete startup', description: 'Delete an existing startup by their unique ID. Requires admin privileges.' })
    @ApiParam({
      name: 'id',
      required: true,
